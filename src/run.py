...
def store_as_json(data, filename):
    try:
        # 1. Extract the directory path from the filename
        directory = os.path.dirname(filename)

        # 2. If a directory path exists (is not empty) and doesn't exist on disk, create it
        if directory and not os.path.exists(directory):
            os.makedirs(directory, exist_ok=True)
            print(f"Created missing directory: {directory}")

        # 3. Open the file in 'w' mode (creates file if missing, truncates if exists)
        ...
        print(f"Serialization Error: Ensure all data types are JSON compatible. {e}")
