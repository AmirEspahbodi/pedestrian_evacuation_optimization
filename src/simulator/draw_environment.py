...
class DomainVisualizerWindow(QMainWindow):
    ...
    def updateGrid(self):
        ...
        if hasattr(self, 'grid_widget'):
            ...
            occupied_count = sum(1 for y in range(self.domain.height) 
                               for x in range(self.domain.width) 
                               if self.domain.cells[y][x].state == CAState.OCCUPIED)
            self.status_bar.showMessage(f"Step {self.step_count} - Occupied cells: {occupied_count}")
    ...
    def initUI(self):
        ...
        self.info_label = QLabel(f"Grid Size: {self.domain.width} x {self.domain.height}")
