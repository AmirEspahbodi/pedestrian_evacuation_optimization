...
class EnvironmentGenerator:
    ...
    def generate_environments(
        self, count: int, location_type: LocationType = LocationType.SUPERMARKET
    ) -> List[Environment]:
    ...
    def _generate_single_environment(
        self, env_id: int, location_type: LocationType
    ) -> Environment:
        ...
        obstacles = self._generate_obstacles(
            width, height, obstacle_grid, location_type
        )
    ...
    def _generate_obstacles(
        ...
        while obstacle_count < target_obstacles and attempts < max_attempts:
            ...
            position = self._find_valid_position(
                width, height, obs_width, obs_height, obstacle_grid
            )
    ...
    def _generate_exits(
        self, width: int, height: int, obstacle_grid: np.ndarray
    ) -> List[Access]:
        ...
        for i in range(num_exits):
            ...
            while attempts < max_attempts:
                ...
                for existing_pa in exit_positions:
                    distance = min(
                        abs(pa - existing_pa), perimeter - abs(pa - existing_pa)
                    )
    ...
    def _is_exit_clear(
        self, pa: int, wa: int, width: int, height: int, obstacle_grid: np.ndarray
    ) -> bool:
                ...
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        ...
                        if (
                            ...
                            and obstacle_grid[check_y, check_x]
                        ):
                            return False

        return True

    def _perimeter_to_coords(
        self, pos: int, width: int, height: int
    ) -> Optional[Tuple[int, int]]:
    ...
    def save_environments_to_json(self, environments: List[Environment], filename: str):
        ...
        for env in environments:
                ...
                obstacles_dict.append(
                    {
                        ...
                        "shape": {
                            "type": obs.shape.type,
                            "topLeft": {
                                "x": obs.shape.topLeft.x,
                                "y": obs.shape.topLeft.y,
                            },
            ...
            for acc in env.accesses:
                accesses_dict.append({"id": acc.id, "pa": acc.pa, "wa": acc.wa})
        ...
        print(
            f"Successfully generated {len(environments)} environments and saved to {filename}"
        )
...
def main():
    ...
    generator = EnvironmentGenerator(
        min_width=60, max_width=100, min_height=40, max_height=80
    )
    ...
    print(
        f"Generating {SimulationConfig.num_simulations} {'rectangle'} environments..."
    )
    environments = generator.generate_environments(
        SimulationConfig.num_simulations, location_type
    )

    # Save to JSON
    generator.save_environments_to_json(
        environments, f"./dataset/environments/environments_{location_type.value}.json"
    )
    ...
    for env in environments:
        print(
            f"Environment {env.id}: {env.width}x{env.height}, "
