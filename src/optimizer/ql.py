...
def _generate_initial_state(k: int, max_pos: int, omega: int) -> Optional[List[int]]:
    ...
    for pos in possible_positions:
        if all(not _are_exits_overlapping(pos, existing_pos, omega) for existing_pos in config):
            config.append(pos)
            if len(config) == k:
...
def q_learning_exit_optimizer(
    ...
    for episode in range(num_episodes):
        ...
        if current_solution is None:
            raise ValueError(f"Cannot place {k_exits} non-overlapping exits of width {omega} "
        ...
        for step in range(max_steps_per_episode):
            ...
            if random.uniform(0, 1) < exploration_rate_epsilon:
                ...
                for idx_to_change in range(k_exits):
                    for new_pos in range(max_val_for_element + 1):
                        ...
                        other_exits = current_solution[:idx_to_change] + current_solution[idx_to_change+1:]
                        if all(not _are_exits_overlapping(new_pos, p, omega) for p in other_exits):
            ...
            if current_fitness < best_fitness:
                ...
                print(f"Episode {episode+1}, Step {step+1}: New best fitness: {best_fitness:.4f} -> {best_solution}")


        # Decay epsilon after each episode to shift from exploration to exploitation
        exploration_rate_epsilon = max(min_exploration_rate, exploration_rate_epsilon * exploration_decay_rate)
