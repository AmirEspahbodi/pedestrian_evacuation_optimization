...
def _gaussian_perturbation(value: float, gamma: float, perimeter_length: float) -> float:
...
def _set_based_recombination(
    parent1_genes: List[float],
    parent2_genes: List[float],
    k_exits: int
) -> List[float]:
    """
...
def _mutate_individual_ea(
    ...
    individual_genes[exit_to_mutate_idx] = _gaussian_perturbation(
        individual_genes[exit_to_mutate_idx],
        mutation_gamma,
        perimeter_length
    )
...
def evolutionary_algorithm(
    ...
    for ind in population:
        if psi_evaluator.get_evaluation_count() >= max_evals: break
    ...
    while psi_evaluator.get_evaluation_count() < max_evals:
        ...
        for _ in range(population_size // 2):
            ...
            if random.random() < recombination_prob:
                offspring1_genes = _set_based_recombination(parent1.genes, parent2.genes, k_exits)
                offspring2_genes = _set_based_recombination(parent2.genes, parent1.genes, k_exits) 
            else:
                offspring1_genes = parent1.genes[:]
                offspring2_genes = parent2.genes[:]

            _mutate_individual_ea(offspring1_genes, mutation_gamma, perimeter_length, k_exits)
            _mutate_individual_ea(offspring2_genes, mutation_gamma, perimeter_length, k_exits)
    ...
    return best_overall_individual.genes, best_overall_individual.fitness, psi_evaluator.get_evaluation_count()