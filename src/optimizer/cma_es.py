...
def adapted_cma_es(
    ...
    fitness_function = FitnessEvaluator(domain=domain, optimizer_strategy=OptimizerStrategy.IEA)
    ...
    while evaluation_count < max_evaluations:
        ...
        for individual in population:
            # --- Projection to valid, non-overlapping exit positions ---
            # Sort to handle overlaps sequentially
            sorted_individual = np.sort(individual)
            
            # Clip to bounds [0, perimeter_length - omega]
            projected = np.clip(sorted_individual, 0, perimeter_length - omega)
            
            # Enforce minimum distance (omega) between exits
            for i in range(1, k_exits):
                if projected[i] < projected[i-1] + omega:
                    projected[i] = projected[i-1] + omega
            
            # Re-clip in case the overlap adjustment pushed values out of bounds
        ...
        if generation == 1 or fitness_values[sorted_indices[0]] < best_fitness:
        ...
        # Recombination of parent population
        old_mean = mean.copy()
        ...
        h_sig = (np.linalg.norm(ps) / np.sqrt(1 - (1 - cs) ** (2 * evaluation_count / population_size))
                 / k_exits < (1.4 + 2 / (k_exits + 1)))
        
        pc = (1 - cc) * pc + h_sig * np.sqrt(cc * (2 - cc) * mueff) * (mean - old_mean) / sigma
        
        # Update covariance matrix
        artmp = (1 / sigma) * (np.array(selected_individuals) - old_mean)
        C = ((1 - c1 - cmu) * C
             + c1 * (pc[:, np.newaxis] @ pc[np.newaxis, :]
                     + (1 - h_sig) * cc * (2 - cc) * C)
             + cmu * (artmp.T @ np.diag(weights) @ artmp))

        # Update step-size
        sigma *= np.exp((cs / damps) * (np.linalg.norm(ps) / np.linalg.norm(np.random.randn(k_exits)) - 1))
        
        # --- 5. Covariance Matrix Decomposition (for efficiency) ---
        if evaluation_count - getattr(C, 'eigen_eval', 0) > population_size / (c1 + cmu) / k_exits / 10:
